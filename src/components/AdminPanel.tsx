...
import { Users, Shield, Check, X, Settings, Trash2, Plus, Edit, Save, UserX, Key } from 'lucide-react';
...
export function AdminPanel() {
  ...
  const deleteUser = async (userId: string) => {
    ...
    try {
      ...
      // Delete from auth.users (this will cascade to other references)
      const { error } = await supabase.auth.admin.deleteUser(userId);
      if (error) throw error;
      
      // Update local state
      setUsers(prev => prev.filter(user => user.user_id !== userId));
      setPermissions(prev => prev.filter(perm => perm.user_id !== userId));
      
      alert('User deleted successfully');
    } catch (error) {
      console.error('Error deleting user:', error);
      alert('Error deleting user. You may not have sufficient permissions.');
    }
  };
  ...
  const handleAddPermission = async () => {
    ...
    try {
      ...
      alert('Error creating permission. This user may already have access to this campaign.');
    }
  };

  const deletePermission = async (permissionId: string, userEmail: string, campaignName: string) => {
    if (!confirm(`Remove ${userEmail}'s access to ${campaignName}?`)) return;
            ...
            <div className="space-y-4">
              {permissions.map((permission) => (
                <div key={permission.id} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      onClick={() => deletePermission(permission.id, permission.user_email!, permission.campaign_name!)}
              ...
              {permissions.length === 0 && (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-600">No campaign permissions configured</p>
